 ...
 * Program Name:            sptMaintenance.php<br>
 * Development Reference:   LP0052<br>
 * Description:             Strategic Planning Team - person list maintenance<br>
 *
 *  MODIFICATION CONTROL<br>
 *  ====================<br>
 *    FIXNO     BY    DD/MM/YYYY  COMMENT<br>
 *  --------  ------  ----------  ----------------------------------<br>
 *   LP0052     AD    29/10/2018  Create new LPS ticket type �Supersession�)*
 *
 */
/**
 */
...
if($_REQUEST['action'] == "edit"){
    ...
    echo "<br />";
    echo "<center> Strategic Planning Team Member List Maintenance </center>";
    echo "<br />";
...
function listLinks(){
    ...
    $sql .= " where ASCII(LEVL22)>96 ";//a
    $sql .= "   and ASCII(LEVL22)<123 ";//z
  ...
  //     echo " <a href='sptMaintenance.php?linkID=" . trim($row['LEVL22']) . "&action=edit' target='_new'>";
  //      echo "   <img src='" . $IMG_DIR . "/edit.gif' border=0 alt='Edit Role' />";
  //      echo " </a>";
  //      echo " &nbsp; ";
        echo " <a href='sptMaintenance.php?linkID=" . trim($row['LEVL22']) . "&action=delete' target='_new'>";
...
function addLink(){
    ...
    echo "<form method='POST' enctype='multipart/form-data' action='sptMaintenance.php'>";
    echo "<table style='width: auto; margin-left: 1%;'>";
    
    echo "<tr>";
    echo "<td> </td>";
    echo "<td> New SPT Member </td>";
    echo "<td> Role  </td>";
    echo "<td> </td>";
    echo "</tr>";
...
function addLinkCIL22($link, $role){
    ...
    $newID='a';
    $sql   = "select (max(ASCII(LEVL22)))as MAXID from CIL22 where ASCII(LEVL22)>96 and ASCII(LEVL22)<123";
    $res = odbc_prepare($conn, $sql);
    odbc_execute($res);
...
function updateLinkCIL22($idLink, $link, $description, $comment, $isLink){
    ...
    //close window:
    echo "<script>";
    echo "  window.opener.location = ('sptMaintenance.php');";
...
function deleteLinkCIL22($idLink){
    ...
    {
        $sqld   = "select (max(ASCII(LEVL22)))as MAXID from CIL22 where ASCII(LEVL22)>96 and ASCII(LEVL22)<123";
